          ...
          <View style={styles.linksWrapper}>
            {topics.map((topic, index) => (
              <Animated.View key={topic.id} entering={FadeInDown.duration(300)} exiting={FadeOutUp.duration(200)} layout={Layout.springify().delay(index * 20)}>
                <Link href={`/systemDesign/${topic.id}`} asChild key={topic.id}>
...
const styles = StyleSheet.create({
  ...
  iconWrapper: {
    ...
    marginLeft: 20,
    borderWidth: 1,
    borderColor: "rgba(75, 85, 99, 0.5)", // gray-700/50
    backdropFilter: "blur(4px)", // This doesn't work in RN, visual fallback only
  },
  ...
  scrollContainer: {
    maxHeight: MAX_ROWS * LINK_HEIGHT, // Show max 3 rows
    maxWidth: "100%",
    paddingVertical: 8,
    marginHorizontal: 10,
  },
