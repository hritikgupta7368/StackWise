...
export const AddSubTopicForm = ({ topicName, id, index, addAtIndex = false }) => {
  ...
  const { addCoreSubtopic } = useAppStore();
  ...
  const handleAdd = () => {
    ...
    addCoreSubtopic(newSubTopic, { index, addAtIndex });
...
export const DeleteSubTopicSheet = ({ topicName }) => {
  const { getCoreSubtopicsByTopicId, deleteCoreSubtopic } = useAppStore();
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const { hideBottomSheet, showToast } = useOverlayStore();
  const subtopics = getCoreSubtopicsByTopicId(topicName);
  ...
  const handleDelete = useCallback(() => {
    Array.from(selectedIds).forEach(deleteCoreSubtopic);
    hideBottomSheet();
