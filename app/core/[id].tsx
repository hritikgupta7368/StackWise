...
import { AddSubTopicForm, DeleteSubTopicSheet } from "@/components/Forms/coreForms/CoreSubTopic";
...
export default function CorePage() {
  ...
  // 1) Get the raw arrays once:
  const allTopics = useAppStore((s) => s.core.topics);
  const allSubtopics = useAppStore((s) => s.core.subtopics);

  // 2) Filter *locally* with useMemo
  ...
  // 3) Keep track of expanded cards
  ...
  // 4) Update header title only when it really changes
  useLayoutEffect(() => {
    ...
    (subId: string, at: number) => {
      showBottomSheet({
        ...
        content: (
          <AddBlockForm
            onSubmit={(newBlock) => {
              useAppStore.getState().insertBlockInSubtopic(subId, at, newBlock);
              hideBottomSheet();
            }}
          />
        ),
      ...
      showBottomSheet({
        ...
        content: (
          <EditBlockForm
            initialBlock={block}
            onSubmit={(updated) => {
              useAppStore.getState().updateBlockInSubtopic(subId, idx, updated);
              hideBottomSheet();
            }}
          />
        ),
  ...
  const handleDeleteBlock = useCallback((subId: string, idx: number) => {
    useAppStore.getState().deleteBlockInSubtopic(subId, idx);
  }, []);
    ...
    <View style={styles.page}>
      ...
      {!subtopics || subtopics.length <= 0 ? (
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center", padding: 20 }}>
          <Text style={{ color: "#fff", fontSize: 16 }}>No subtopics found</Text>
        </View>
      ) : (
        <View style={{ flex: 1, paddingHorizontal: 14, paddingTop: 7 }}>
          <FlashList
            data={subtopics}
            extraData={expandedIds} // ← forces re-render when expand state changes
            keyExtractor={(item) => item.id}
            renderItem={({ item, index }) => <SubTopicCard index={index} getIndex={() => addSubtopicAtIndex(item.id, index)} subtopic={item} isExpanded={expandedIds.has(item.id)} onToggle={() => toggleExpanded(item.id)} onAddBlock={() => handleAddBlock(item.id, item.content.length)} onEditBlock={(blk, idx) => handleEditBlock(item.id, blk, idx)} onDeleteBlock={(idx) => handleDeleteBlock(item.id, idx)} />}
